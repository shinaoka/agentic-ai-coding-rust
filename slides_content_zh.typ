...
#{
  ...
  table(columns: (25%, 1fr), stroke: none, row-gutter: 0.34em, column-gutter: 0.75em, align: (left + top, left + top),
    ...
    [#text(size: 0.82em, style: "italic")[“…would prefer not to use agentic tools #hl[even if they worked as advertised].”]\ #text(size: 0.72em, fill: gray)[即便工具能工作, 人也会失去对工作的关心. #lk("https://sinclairtarget.com/blog/2026/06/01/quality-in-the-age-of-slop/", "Quality in the Age of Slop, 2026")]],

    [*维护崩溃*\ #text(size: 0.72em)[D. Stenberg (curl)]],
    [#text(size: 0.82em, style: "italic")[“The current torrent of submissions put a high load on the curl security team…”]\ #text(size: 0.72em, fill: gray)[curl 被 AI slop 淹没, 关闭了 bug bounty (2026). #lk("https://www.theregister.com/2026/01/21/curl_ends_bug_bounty/", "The Register")]],
    ...
    [#text(size: 0.82em)[用 16 个并行 Claude 实例构建 C 编译器 (100K 行, ~\$20k). 独立基准测试显示它 #hl[快于 GCC -O0, 但仍落后于 GCC -O2]. 评价目标不清楚时, 这类展示会 #hl[扩大对 AI 的怀疑].]\ #text(size: 0.72em, fill: gray)[#lk("https://www.anthropic.com/engineering/building-c-compiler", "Anthropic") · #lk("https://dineshgdk.substack.com/p/benchmarking-claude-c-compiler", "benchmark")]],
  )
}
...
#v(0.3em)
人类一边与 AI 讨论, 一边设计分离结构. #hl[一旦分层完成, AI 只能在各层内部活动, 不能破坏层次结构.]
#ref-text[层和 API 边界是与 AI 一起详细分析 PyTorch / JAX 的设计 (成功与失败) 后决定的.]
