...
#v(0.2em)
#align(center, text(size: 0.92em)[$arrow.r$ いずれも #hl[人手で支えるモデル] が抱える限界．])
...
== 私の解答
#v(1fr)
いずれも agentic coding の本質的な限界ではなく，#hl[AI を前提に体制を組み替えれば解消する] 移行期の課題である．
#v(0.6em)
#table(columns: (auto, 1fr), stroke: none, row-gutter: 0.9em, column-gutter: 1.2em, align: (left + top, left + top),
  [*職人性の喪失*\ #text(size: 0.78em, fill: muted)[Sinclair Target]],
  [伝統工芸と同じく，手動コーディングが残り，意味を持つのは #hl[ごく限られた領域] に過ぎない．],

  [*保守の崩壊・共有資源の劣化*\ #text(size: 0.78em, fill: muted)[D. Stenberg, Baltes et al.]],
  [手動の PR・マージを前提とした運用を #hl[維持する必要はない]．AI を前提としたメンテナンス体制へ移行すべきであり，#hl[人間の関与はむしろボトルネック] である．],

  [*過大広告への反動*\ #text(size: 0.78em, fill: muted)[Anthropic C compiler]],
  [最適化不足は #hl[AI 不能の証拠ではない]．性能 oracle と緊密な human-AI loop の不足として読むのが妥当である (後述)．],
)
#v(1.2fr)
...
#v(0.5em)
#text(size: 0.88em)[
  AI の高速生成は工業的な大量生産である．製品を1個ずつ直すのではなく，#hl[金型 (= 正本: rules · oracle)] を直す.\
...
#v(0.2em)
近年は両者の境界が薄れ，#hl[エージェント型が両形態を飲み込みつつある]．GUI から自律実行を呼ぶ Codex App はその典型である．
...
- #hl[CLAUDE.md / AGENTS.md] = セッションを越える記憶 (規範・コマンド・構成)．
  - リポジトリ内に置く #hl[プロジェクト固有] の情報であり，git でチームや将来の自分と共有する (ユーザ全体の設定とは別)．
...
#table(columns: (auto, 1fr), stroke: none, row-gutter: 0.4em,
  ...
  [*2026年3月〜*], [Codex, Claude, DeepSeek, Kimiなどを使い分け開発を進める],
)
...
- Juliaによる大規模開発の辛さ: (pre)compileの遅さ，実行時に出る型不安定性等
...
最初は「モノシリック構造」から始めた:
- ITensors.jlの必要部分 (Index system, tensor contraction)
- QTT/TCI
の移植
...
#ref-text-inline[#lk("https://skybrary.aero/articles/heinrich-pyramid", "SKYbrary: Heinrich Pyramid") · #lk("https://www.icao.int/sites/default/files/postalhistory/annex_13_aircraft_accident_and_incident_investigation.htm", "ICAO Annex 13") · #lk("https://asrs.arc.nasa.gov/", "NASA ASRS")]
...
#v(0.1em)
#ref-text[AI とともに PyTorch / JAX を分析して設計． #text(style: "italic")[(Jin-Guo Liu: tenferro 初期設計, Satoshi Terasaki: 開発支援, tensor4all collaboration: tensor4all-rs / Tensor4all.jl 開発)]]

== 各階層のモジュール化

#align(center, image("figures/tenferro-architecture.svg", width: 80%))
#v(0.2em)
#align(center, text(size: 0.85em)[Rust はモジュール/クレート階層に沿って #hl[公開/非公開] を制御可能])

// 規模の壁 + ChatGPT解答 (三層構造) は後半「正本を育てる」の三本柱へ移動した．


== レファクタリングを繰り返す
#align(center, image("figures/tenferro-commits.png", width: 70%))

#align(center, text(size: 0.85em)[大規模なリファクタリングを繰り返して品質を上げる])
...
最近は Jupyter Notebook でプロトタイプを作ることが多く, 数式とコードが似ていてレビューしやすく, メモリ管理に気を使わなくて良いJulia が楽．Rust にも興味はあるが, 一般の学生が覚えるには敷居が高く, 数値計算ライブラリもまだ揃っていない．

=== 2026年
最近はプロトタイピングに Jupyter Notebook は使わない． メモリ管理が厳格で明示的な Rust の方が agentic coding には効率的である．エージェントがコードを書くので, 学習曲線の急峻さも問題ない．数式との整合も AI の助けがあればコードが少々長くても問題ない．
...
#align(center, text(size: 1.0em)[#hl[ではどの言語が検証可能性に向くのか]．])
...
#align(center, {
  let chip(body) = box(
    ...
    radius: 5pt,
    stroke: rgb("#cc0000") + 1.2pt,
    fill: rgb("#cc0000").lighten(92%),
  )[#align(center, text(weight: "bold", fill: rgb("#cc0000"), size: 0.85em)[#body])]
  let plus = text(size: 1.3em, weight: "bold", fill: muted.darken(45%))[#sym.plus]
  grid(columns: 5, align: horizon, column-gutter: 0.5em,
    chip[C++ 並の \ 高速性],
    plus,
    chip[Julia のような \ ビルド/依存管理],
    plus,
    chip[メモリ管理 \ の厳密さ],
  )
})

- #hl[Mozilla] 発のシステム言語．ブラウザエンジン (Servo) のために開発され，2015 に安定版 (1.0)．
...
#text(size: 0.86em)[同時に持てるのは #hl[`&T` を複数] (共有・読取) #h(0.2em)*または*#h(0.2em) #hl[`&mut T` を1個だけ] (排他・書込)．両立しない．]
#v(0.1em)
...
#let datbox(c) = {
  import cetz.draw: *
  rect((3.0, -0.5), (4.5, 0.5), radius: 0.12, fill: c.lighten(88%), stroke: c + 1pt)
  content((3.75, 0), text(size: 0.85em)[`v`])
}
#let panel(ok, title, cap, fig) = {
  let c = if ok { okc } else { ngc }
  let mark = if ok { "✓" } else { "✗" }
  block(width: 100%, inset: 0.4em, radius: 6pt, stroke: c + 1.3pt)[
    #set align(center)
    #text(weight: "bold", size: 0.88em, fill: c)[#mark #title]
    #v(0.12em)
    #fig
    #v(0.1em)
    #text(size: 0.72em, fill: muted.darken(45%))[#cap]
  ]
}

#grid(columns: (1fr, 1fr), column-gutter: 1em, row-gutter: 0.45em,
  panel(true, "不変借用は複数OK", [`&v` は同時に何個でも持てる (読取専用)], cetz.canvas(length: 0.58cm, {
    import cetz.draw: *
    hide(rect((-1.3, -1.15), (-1.29, 1.15)), bounds: true)
    datbox(okc)
    content((1.0, 0.72), text(size: 0.72em, fill: okc)[`&v`], anchor: "east")
    content((1.0, -0.72), text(size: 0.72em, fill: okc)[`&v`], anchor: "east")
    line((1.3, 0.62), (2.95, 0.26), mark: (end: "stealth"), stroke: okc + 1pt)
    line((1.3, -0.62), (2.95, -0.26), mark: (end: "stealth"), stroke: okc + 1pt)
  })),
  panel(true, "可変借用は1個だけ", [`&mut v` は同時に1個 (排他・書込可)], cetz.canvas(length: 0.58cm, {
    import cetz.draw: *
    hide(rect((-1.3, -1.15), (-1.29, 1.15)), bounds: true)
    datbox(okc)
    content((1.0, 0.0), text(size: 0.72em, fill: okc)[`&mut v`], anchor: "east")
    line((1.3, 0.0), (2.95, 0.0), mark: (end: "stealth"), stroke: okc + 1.3pt)
  })),
  panel(false, "可変 + 不変 の同時", [書込み中に別名から読める → 禁止], cetz.canvas(length: 0.58cm, {
    import cetz.draw: *
    hide(rect((-1.3, -1.15), (-1.29, 1.15)), bounds: true)
    datbox(ngc)
    content((1.0, 0.72), text(size: 0.72em, fill: ngc)[`&mut v`], anchor: "east")
    content((1.0, -0.72), text(size: 0.72em, fill: ngc)[`&v`], anchor: "east")
    line((1.3, 0.62), (2.95, 0.26), mark: (end: "stealth"), stroke: ngc + 1pt)
    line((1.3, -0.62), (2.95, -0.26), mark: (end: "stealth"), stroke: ngc + 1pt)
  })),
  panel(false, "可変借用が2個", [二重の書込み権 → 禁止], cetz.canvas(length: 0.58cm, {
    import cetz.draw: *
    hide(rect((-1.3, -1.15), (-1.29, 1.15)), bounds: true)
    datbox(ngc)
    content((1.0, 0.72), text(size: 0.72em, fill: ngc)[`&mut v`], anchor: "east")
...
#v(0.35em)
#align(center, text(size: 0.86em)[#sym.arrow.r aliasing と書込みの両立を #hl[*コンパイル時*] に排除．データ競合・dangling が原理的に起きない．])

== エコシステムの急成長
#v(-0.4em)
#align(center, image("figures/fig_crate_growth.svg", width: 56%))
#v(0.5em)
#align(center, text(size: 0.85em)[2026 年には NVIDIA も Rust 製 CUDA カーネルコンパイラ #lk("https://github.com/NVlabs/cuda-oxide", `cuda-oxide`) を公開 (実験段階)．])
///#v(-0.2em)
...
== 代表的なパッケージ (科学計算)
#v(0.3em)
#table(columns: (auto, 1fr), stroke: none, row-gutter: 0.55em, column-gutter: 1.2em, align: (left + horizon, left + horizon),
  [#lk("https://crates.io/crates/ndarray", `ndarray`) / #lk("https://crates.io/crates/ndarray-linalg", `ndarray-linalg`)], [NumPy 的な多次元配列 + BLAS/LAPACK],
  [#lk("https://crates.io/crates/nalgebra", `nalgebra`) / #lk("https://crates.io/crates/faer", `faer`)], [線形代数 (faer は高速)],
  [#lk("https://crates.io/crates/rayon", `rayon`)], [スレッド並列 (共有メモリ, 1行で並列化)],
  [#lk("https://crates.io/crates/mpi", `mpi`) (rsmpi)], [分散並列 (プロセス間・ノード間)],
  [#lk("https://crates.io/crates/num-complex", `num-complex`)], [複素数],
  [#lk("https://crates.io/crates/burn", `Burn`)], [Rust 製 深層学習/テンソル (autodiff・GPU)],
  [#lk("https://crates.io/crates/hdf5-metno", `hdf5-metno`)], [HDF5 入出力],
)
  #text(size: 0.9em)[線形演算の自動微分をサポートするライブラリがなかったので，tenferroの開発を始めた．]
...
- AI が ownership / lifetime の *機械的複雑さ* を肩代わり．
  - 人間は #hl[アルゴリズム・設計・正しさ] の検証に集中できる．
- #hl[逐行レビューはしない]．それでも正しさの確信は手作業時代より高い．

$arrow.r$ Julia/C++/Python で感じていた「大規模化すると検証が難しくなる」という不安が #hl[なくなった]．
...
#align(center + horizon)[#text(size: 1.15em)[
  選ぶ基準は #hl[認知コスト最小化] から #hl[検証可能性最大化] へ
]]
...
== 巨大・高機能なコードを3本柱で支える
tenferro-rs は #hl[約13万行]，einsum · FFT · 自動微分 (AD) · GPU まで担う高機能スタック．#hl[v0.1 を公開] (フル機能は実装済み, ただし API 未確定の初期版)．
...
#ref-text[#lk("https://tensor4all.org/blog/introducing-tenferro-rs/", "tensor4all.org/blog: Introducing tenferro-rs (v0.1, 2026)")]
...
- #hl[Rust が効く点]: 依存 crate を *並列コンパイル*，テストも *デフォルトで並列実行*．
  - 実例 (tenferro-rs, Macbook Pro): #hl[フルスクラッチビルド ~2分] ・ cargo check 瞬時 ・ AI edit→該当 test 数十秒．
...
#grid(columns: (54%, 46%), gutter: 1em, align: (left + horizon, center + horizon),
  [
    ...
    - 正本は3本柱で #hl[最も過小評価されやすい]．次スライドから #hl[育て方] を扱う．
  ],
...
#align(center)[
  ...
  正本 + コード \
  #sym.arrow.b \
  *監査 Bot* を private CI で継続実行 #text(size: 0.8em, fill: muted)[(人間も同ツール可)] \
  #sym.arrow.b \
  不整合・バグを #hl[Issue に自動起票] \
  #sym.arrow.b \
  *人間* が GPT-5.5 で #hl[一括修正]
]
#v(1fr)
#ref-text[自動修正に任せきらないのは #hl[信頼性ではなく責任と選択の問題]: 多数のバグの背後に設計の不具合があることもある ．]
...
研究室 (埼玉大) で #hl[Rust 計算物理学チュートリアル] を作成中．
#ref-text-inline[#lk("https://saitama-cond-mat.github.io/rust-computational-physics-tutorial/", "saitama-cond-mat.github.io/rust-computational-physics-tutorial")]
#v(0.3em)
- 対象: 物理の基礎がある #hl[学部上級〜大学院初級] (プログラミングは初級でよい)．
- 内容: 基礎・開発環境 $arrow.r$ 数値解法 (微分方程式・線形代数・Fourier・Monte Carlo) $arrow.r$ 物理シミュレーション (力学・流体・統計力学・量子) $arrow.r$ 並列化・最適化．
- 方針: #hl[ルーチンな実装は AI に任せ, 物理的・数値的な正しさは人間が検証する]．
#v(0.3em)
#align(center, text(size: 1.05em)[$arrow.r$ 本日の方法論を, #hl[計算物理の教材] として実装しようとしている (貢献歓迎！）．])

== 論点: AI 時代の OSS 共同開発はどうなるか
*① Issue と PR* (どちらも pile up していた)
- #hl[Issue の価値は上がる]: バグ報告・新機能要望 = #hl[意図と仕様]．AI には生み出せない部分．
- #hl[PR の役割は縮む]: 小規模修正には有効．大規模な最適化は #hl[Issue を起点に主開発者 + AI が一貫実装] する方が速く, 整合的．
 - 前向きに言えば, #hl[良い Issue を集める] ことが共同開発の中心になる．
#v(0.3em)
*② コードの共用* (AI でコピー・移植が容易に)
- #hl[基礎ライブラリの乱立は避けたい]: 信頼性と検証コストの問題．少数を皆で育て, oracle / test で支える．
- #hl[応用に近い層は共用の必要が薄れる]: 各自がコピーして改変すればよい．
#v(0.4em)
#align(center, text(size: 1.05em)[$arrow.r$ PR も応用コードも共有価値が下がるなら, #hl[コミュニティは何を共有するのか]．])
...
// ============================================================
= ハンズオン: 2D Isingで検証駆動開発
// ============================================================

== セットアップ: 最初に AI に指示すること
- 最初に #hl[Superpowers] を導入: AI に #hl[URL を渡して「インストールして」と頼むだけ] (#lk("https://github.com/obra/Superpowers", "github.com/obra/Superpowers"))．brainstorm → plan → execute を強制する skill 群．
