...
#table(columns: (auto, 1fr), stroke: none, row-gutter: 0.7em, column-gutter: 0.9em, align: (left + top, left + top),
  ...
  [#text(size: 0.85em)[16 並列 Claude で C コンパイラ (100K 行, ~\$20k) も #hl[GCC -O0 より遅い]．こうした under-baked な showcase が #hl[AI 反対派を増やす]．]\ #text(size: 0.78em, fill: gray)[#lk("https://www.anthropic.com/engineering/building-c-compiler", "Anthropic") · #lk("https://dineshgdk.substack.com/p/benchmarking-claude-c-compiler", "benchmark")]],
)
#ref-text[ただし C コンパイラの遅さは #hl[AI 不能の証拠ではなく], 性能 oracle と緊密な human-AI loop の欠如が原因 (後述)．]
...
#table(columns: (auto, 1fr), stroke: none, row-gutter: 0.45em,
  ...
  [#lk("https://github.com/badlogic/pi-mono/tree/main/packages/coding-agent", "Pi")], [最小構成の OSS. 拡張・skill を自分で足して育てる (最小主義者向け)],
)
...
最近はJupyter Notebookでプロトタイプを作ることが多いですし，数式とコードが似ていてレビューしやすく，メモリ管理を良きようにやってくれるJuliaが楽ですね．Rustも興味ありますけど，一般の学生が覚えるのには敷居が高いですし，数値計算ライブラリもまだまだ揃っていないですよね．

=== 2026年
最近はプロトタイピングにJupyter Notebookは使わないし，メモリ管理が厳格で明示的なRustの方がエージェントコーディングには効率的ですね．エージェントがコードを書くので，学習曲線の急峻さも問題ない．数式との整合もAIの助けがあればコードが少々長くても問題ないです．
足りない数値ライブラリはRustに簡単に移植できます．
...
#text(size: 1em)[#super[\*]巨大なコードが前提です.]
...
- #hl[Rust が効く点]: 依存 crate を *並列コンパイル*，テストも *デフォルトで並列実行*．数十秒で全テストが回る．
...
- #hl[性能] = tenferro-benchmark / #hl[AD の正しさ] = tensor-ad-oracles → #hl[柱2 (外部基準)] を言い当てた．
- #hl[構造の一貫性] = CI + repository rules + first-class crate 分割 → #hl[柱3 (正本)] を言い当てた．
#v(0.2em)
...
#table(columns: (auto, 1fr), stroke: 0.4pt + gray, inset: 8pt,
  [*① library ≠ application*], [application は reference answer が無く, 符号・prefactor のバグが支配的],
...
#ref-text[既存教材 #lk("https://github.com/saitama-cond-mat/rust-computational-physics-tutorial", "rust-computational-physics-tutorial") の Ising 章が下敷き．]
...
#ref-text[テンプレ: 問題 / 定式化 / 仮定 / algorithm / 擬似コード / invariants / reference / error metrics / test plan]
...
Task 1 で分かったことを #hl[次のセッションへ引き継ぐ] ため rules に固定する:
